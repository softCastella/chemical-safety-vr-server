...
## 2026-09-09 VR 상세페이지 PPE 모드·공유 UI 운영 배포

화학물질 안전훈련 VR 상세페이지가 현재 제공하는 PPE 착용 교육 범위와 교육·훈련·테스트 모드의
차이를 방문자가 확인할 수 있도록 콘텐츠를 보강하고 운영에 반영했다. 설명 근거는 Unity 클라이언트
`softCastella/chemical-safety-vr-client`의 `main@fd925828f63eba564849eea47477a15325b22b7e`와
현재 빌드 씬 `Assets/Scenes/3_PPE_Room_3mode_loco.unity`의 정적 확인 결과다.

- 운영 콘텐츠 기준: `main@134dd8de500f788665e6ac8b5f90fa635b17958a`
- 운영 서버 체크아웃: `/home/linuxuser/workspace/chemical-safety-vr`, `main` 브랜치
- 공개 주소: `https://immersa.tycheworks.com/chemical-safety-training`
- 콘텐츠 변경: PPE 선택·오염 및 손상 판단·착용 순서·거울 확인·5문항 퀴즈의 공통 흐름과 세 학습
  모드의 안내 수준, 오답 처리 및 결과 표시 차이를 추가했다. 밀폐공간과 누출 대응은 향후 확장 가능한
  콘텐츠 정의와 분리해 `현재 시나리오`로 표시했다.
- 공유 UI 변경: 플랫폼 이름과 `LINE` 워드마크를 보조 텍스트 최솟값 11px로 올리고, 깨져 보이던
  링크 경로를 겹친 사각형 복사 아이콘으로 교체했다.

완료 검증은 다음과 같다.

- 로컬과 운영 서버에서 `npm test` 93개 통과, `git diff --check` 통과
- 운영 서버 fast-forward 반영 후 `nginx -t` 통과와 깨끗한 작업 트리 확인
- 공개 HTML에서 PPE 세 모드 안내, 11px 공유 표기, 새 복사 아이콘 경로와 이전 아이콘 경로 제거 확인
- 상세페이지, `detail-share.js`, 공개 카카오 설정 API와 헬스체크 HTTP `200` 확인
- 공개 카카오 설정 API의 `Cache-Control: no-store` 유지 확인
- 정적 파일을 Nginx가 직접 제공하므로 PM2와 Nginx를 재시작하지 않았다. DB 마이그레이션,
  운영 DB·환경 변수와 사용자 데이터도 변경하지 않았다.

브라우저 캐시에 기존 HTML이 남아 있으면 한쪽 고리만 보이는 이전 링크 아이콘이 일시적으로 표시될
수 있다. 강력 새로고침 또는 새 탭에서 새 복사 아이콘을 확인한다. 실제 모바일·데스크톱 브라우저의
공유 모달 레이아웃과 각 외부 공유 서비스 전환은 별도 수동 검증 대상으로 남긴다.
