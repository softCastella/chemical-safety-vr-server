...
- 비정상 접속 이력의 국가는 MaxMind GeoLite2 Country 로컬 데이터베이스(`/var/lib/GeoIP/GeoLite2-Country.mmdb`)에서 조회한다. IP는 외부 업체로 전송하지 않으며, DB 갱신은 `geoipupdate.timer`가 수행한다. 사설·루프백·잘못된 IP, DB에 없는 IP와 DB 읽기 실패는 각각 화면에 상태를 표시하고 감사 로그 조회 자체는 계속한다. VPN·프록시 사용 시 실제 사용자 위치가 아니라 출구 IP의 국가로 판정되므로 국가 정보만으로 접속 원인이나 사용자 위치를 확정하지 않는다.
...
- 국가 열은 감사 로그의 공인 IP를 서버 로컬 GeoLite2 Country DB로 조회한다. 국가 정보는 IP 기반 추정이며 해외 접속 여부나 실제 사용자 위치를 확정하지 않는다.
