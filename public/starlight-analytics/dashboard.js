(function(){
  ...
  const localPreview=new URLSearchParams(location.search).has("local-preview");
  const state={data:null,subscriptions:{data:[],meta:{total:0,active:0}},days:7,heatmap:null,heatmapView:null,miniHeatmaps:[],loadError:false};
  const labels={
    ...
    home_click:"나가기",next_stage_click:"다음 스테이지",village_click:"마을 보기",web:"웹",android:"안드로이드",
  ...
  document.addEventListener("DOMContentLoaded",()=>{
    if(localPreview){$("[data-view='release']")?.remove();$("[data-dashboard-switcher]")?.remove();}
  ...
  async function load(){
    ...
    try{
      ...
      data=localPreview?live:(live.meta?.has_any_data?live:window.STARLIGHT_SAMPLE_DASHBOARD);
      if(releaseResponse.ok)state.subscriptions=await releaseResponse.json();
      state.loadError=false;
    }catch(error){
      console.warn("Starlight dashboard API unavailable.",error);
      state.loadError=localPreview;
      data=localPreview?{meta:{data_state:"error",from:$("#date-from").value,to:$("#date-to").value},overview:{},daily:[],funnel:[],stages:[]}:window.STARLIGHT_SAMPLE_DASHBOARD;
    }
  ...
  function render(data){
    ...
    const stateLabel=state.loadError?"데이터 연결 실패":sample?"샘플 데이터":data.meta?.data_state==="no_data"?"해당 기간 데이터 없음":"실제 수집 데이터";
    $("#data-badge").textContent=stateLabel;
    $("#data-badge").className=`data-badge ${state.loadError?"error":sample?"sample":"live"}`;
    $("#range-note").textContent=`${data.meta?.from||$("#date-from").value} ~ ${data.meta?.to||$("#date-to").value} · ${stateLabel}${localPreview?" · 운영 DB 읽기 전용 로컬 보기":""}${data.meta?.event_count!==undefined?` · 이벤트 ${integer(data.meta.event_count)}건`:""}`;
    fillSelect("#filter-locale",data.filters?.locales,"전체 언어");fillSelect("#filter-source",data.filters?.sources,"전체 유입 경로");
    fillSelect("#filter-campaign",data.filters?.campaigns,"전체 캠페인");fillSelect("#filter-stage",data.filters?.stages,"전체 스테이지");
  ...
  function renderOverview(data){
    ...
    const shown=(count)=>state.loadError?"—":integer(count);
    const cards=[["익명 사용자",shown(value.users),"명 · 선택 기간"],["방문 세션",shown(value.sessions),"회 · 선택 기간"],["게임 열기",shown(value.game_opens),"건 · game_open"],["퍼즐 시작",shown(value.puzzle_starts),"건 · puzzle_start"],["데모 완료",shown(value.demo_completes),"건 · demo_complete"]];
    $("#overview-kpis").innerHTML=cards.map(([label,number,unit])=>`<article class="kpi"><span>${label}</span><strong>${number}</strong><small>${unit}</small></article>`).join("");
    renderJourney(data.funnel||[]);renderOverviewStages(data.stages||[]);renderDaily(data.daily||[]);
  }
  ...
  function renderJourney(rows){
    const selected=["landing_view","game_open","puzzle_start","demo_complete"].map((step)=>rows.find((row)=>row.step===step)).filter(Boolean);
    const maximum=Math.max(1,...selected.map((row)=>row.users||0));
    $("#journey-summary").innerHTML=selected.map((row)=>`<div class="journey-row"><div><b>${escapeHtml(title(row.step))}</b><span>${integer(row.users)}명</span></div><div class="progress"><i style="width:${Math.min(100,(row.users||0)/maximum*100)}%"></i></div></div>`).join("")||emptyState();
  }
  function renderOverviewStages(rows){
    $("#overview-stages").innerHTML=rows.map((row)=>`<div class="overview-stage"><b>스테이지 ${integer(row.stage)}</b><span>시작 ${integer(row.start_users)}명</span><span>완료 ${integer(row.clear_users)}명</span></div>`).join("")||emptyState();
  }
  ...
  function title(value){if(value===null||value===undefined)return"-";return labels[value]||String(value).replace(/^stage_(\d)_(start|clear)$/,(_,stage,action)=>`스테이지 ${stage} ${action==="start"?"시작":"완료"}`).replaceAll("_"," ");}
  ...
  function emptyRow(columns){return `<tr><td colspan="${columns}" class="empty-state">${state.loadError?"데이터 연결에 실패했습니다.":"선택 조건에 해당하는 데이터가 없습니다."}</td></tr>`;}
  function emptyState(){return `<p class="empty-state">${state.loadError?"데이터 연결에 실패했습니다.":"선택 조건에 해당하는 데이터가 없습니다."}</p>`;}
})();
