...
function drawTrend(days) {
  ...
  target.innerHTML = `<svg viewBox="0 0 ${width} ${height}" role="img" aria-label="일별 신규 사용자 및 재이용 사용자 추이"><title>일별 신규 사용자 및 재이용 사용자</title>${grid}<line class="axis" x1="${left}" y1="${top + plotHeight}" x2="${width - right}" y2="${top + plotHeight}"/>${labels}<path class="plot-line line-new" d="${line("newUsers")}"/><path class="plot-line line-return" d="${line("returningUsers")}"/>${dots}</svg>`;
}
...
function render(data) {
  ...
  $("trendSummary").textContent = `기간 내 신규 사용자 ${formatted(data.summary.newUsers)}명 · 플레이 ${formatted(data.summary.plays)}회. 동일 사용자의 여러 날짜 재이용은 날짜마다 한 번씩 표시합니다.`;
}
