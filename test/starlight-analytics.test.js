...
test("세 관리자 대시보드는 공용 전환 메뉴와 로그아웃을 제공한다", async () => {
  ...
  assert.match(vrHtml, /href="overview\.css\?v=20260915-3"/);
  assert.match(vrHtml, /src="overview\.js\?v=20260914-2"/);
  assert.match(vrDetailHtml, /href="dashboard\.css\?v=20260911-2"/);
  assert.match(vrDetailHtml, /src="dashboard\.js\?v=20260915-1"/);
